    ...
    const fetchOwner = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:3000/api/users/${owner}`
        );
  ...
  const handleDelete = async () => {
    try {
      const result = await axios.delete(
        `http://localhost:3000/api/comment/deleteComment/${id}/${video}/${user?._id}`
      );
      if (result) {
  ...
  const handleUpdate = async () => {
    try {
      const result = await axios.put(
        `http://localhost:3000/api/comment/updateComment/${id}/${video}/${user?._id}`,
        {
