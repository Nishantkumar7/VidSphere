...
const VideoView = () => {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:3000/api/video/likeVideo/${video}`,
        { uId },
        {
          headers: {
  ...
  const handleDisLike = async () => {
    ...
    try {
      let uId = user._id;
      const { data } = await axios.put(
        `http://localhost:3000/api/video/disLikeVideo/${video}`,
        { uId },
        {
          headers: {
            Authorization: `JWT ${token}`,
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await axios.get(`http://localhost:3000/api/video/${video}`);
      if (data) {
  ...
  const fetchChannelVideos = async (id) => {
    try {
      const { data } = await axios.get(`http://localhost:3000/api/video/channelVideos/${id}`);
      if (data) {
  ...
  const fetchChannelData = async (cId) => {
    const { data } = await axios.get(`http://localhost:3000/api/channel/${cId}`);
    if (data) {
  ...
  const fetchVideoComments = async () => {
    const { data } = await axios.get(`http://localhost:3000/api/comment/videoComments/${video}`);
    if (data) {
  ...
  const handleComment = async () => {
    ...
    try {
      const data = await axios.post("http://localhost:3000/api/comment/addComment", commentData);
      if (data) {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:3000/api/channel/subscribeChannel/${channelData?._id}/${user?._id}`,
        {},
        {
          headers: {
            Authorization: `JWT ${token}`,
